...
```{r}
#credit_risk contails the outcome we are trying to predict credit risk.
...
```{r}
#getting p=0.8 from lecture
set.seed(111)
trainList <- createDataPartition(y=credit$credit_risk,p=.80,list=FALSE)

train <- credit[trainList,]

test <- credit[-trainList,]
...
```
```{r}
#building a support vector model using caret package:
trctrl = trainControl(method='cv', number = 10)
svm.model <-train(credit_risk ~ .,
                  data = train,
                  method = "svmRadial",
                  trControl = trctrl,
...
```{r}
svmPred <- predict(svm.model,newdata=test)
```
...
```{r}
confusion1 <- confusionMatrix(svmPred,test$credit_risk)
```
